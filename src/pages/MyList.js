...
const MyListArray = Array.from({ length: 60 }, (_, i) => ({
  id: i,
  title: `Program ${i + 1}`,
...
export default function MyList() {
  ...
  useEffect(() => {
    setItems(MyListArray.slice(0, visibleCount));
  }, [visibleCount]);

  // Handle infinite scroll and footer visibility in a single useEffect
  useEffect(() => {
    const handleScroll = () => {
      // Check if we need to load more items
      if (
        window.innerHeight + window.scrollY >= document.body.offsetHeight - 100 &&
        visibleCount < MyListArray.length
      ) {
        setVisibleCount((prev) => {
            const next = Math.min(prev + 8, MyListArray.length);
            return next === prev ? prev + 1 : next; // force re-render if maxed
          });   
      }

      // Footer visibility logic
      const allLoaded = visibleCount >= MyListArray.length;
    ...
    const checkInitialContent = () => {
      ...
      if (visibleCount >= MyListArray.length) {
        setShowFooter(true);
      } else {
